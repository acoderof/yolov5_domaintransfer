...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            r1,_=self.forward(torch.zeros(1, ch, s, s),isdomain=False)
            m.stride = torch.tensor([ s / x.shape[-2] for x in r1])   
    ...
    def forward(self, x, domainx=None,augment=False, profile=False, visualize=False,isdomain=True):
        if augment:
            return self.forward_augment(x,isdomain)  # augmented inference, None
        return self.forward_once(x,domainx ,profile, visualize,isdomain)  # single-scale inference, train

    def forward_augment(self, x,isdomain=True):
        ...
        for si, fi in zip(s, f):
            ...
            yi = self.forward_once(xi,isdomain=isdomain)[0]  # forward
            # cv2.imwrite(f'img_{si}.jpg', 255 * xi[0].cpu().numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
            yi = self._descale_pred(yi, fi, si, img_size)
            y.append(yi)
        return torch.cat(y, 1), None  # augmented inference, train

    def forward_once(self, x,domainx, profile=False, visualize=False,isdomain=True):
        y, dt = [], []  # outputs
        y1=[]
        d1= 0
        d2=0
        ...
        for index, m in enumerate(self.model):
            ...
            x = m(x)  # run
            if(index<15 and isdomain):
                x1=m(x1)
                #print("layer",index,x.shape,x1.shape)
                if(index==6):
                    #print(x.shape,x1.shape,x.shape)
        ...
        #print("forward_once",type(d1),type(d2),type(d3))
        return x,d1+d2+d3
