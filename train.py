...
from utils.datasets import create_dataloader,create_domaindataloader
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_loader, dataset,domain_train_loader,domain_dataset = create_domaindataloader(train_path, domain_path,imgsz, batch_size // WORLD_SIZE, gs, single_cls,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK != -1:
            ...
            domain_train_loader.sampler.set_epoch(epoch)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #print("xxxxxxxx ",imgs.shape," " ,i)
            domainlist= next(iterr)
            domainimgs = domainlist[0]
            if(domainimgs.shape[0]<imgs.shape[0]):
                domainlist= next(iterr)
                domainimgs = domainlist[0]
            #print("xxxxxxxx[domain] ",len(domainimgs)," " ,domainimgs[0].shape)
            domainimgs = domainimgs.to(device, non_blocking=True).float() / 255.0
            ...
            with amp.autocast(enabled=cuda):
                pred,domain_loss = model(imgs,domainimgs)  # forward
            ...
            if RANK in [-1, 0]:
                ...
                #print("mloss",type(mloss),mloss.shape)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/cityspaces.yaml', help='dataset.yaml path')
